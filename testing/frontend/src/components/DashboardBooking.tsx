...
const DashboardBooking: React.FC = () => {
  ...
  const [step, setStep] = useState(1);           // текущий шаг (1..3)
  const [isLoading, setIsLoading] = useState(true);
  const [bookingSuccess, setBookingSuccess] = useState(false);
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");    // Для возможных уведомлений

  // Эффект для автосброса message (как в DashboardUpload)
  useEffect(() => {
    if (message) {
      ...
      const mockCars: Car[] = [
        {
          ...
          year: 2022,
          price: 45,
          image: "https://via.placeholder.com/300x200?text=Toyota%20Camry",
          owner: "John D.",
          location: "Almaty, Center",
          available: true,
        },
        {
          id: "car2",
          ...
          year: 2021,
          price: 40,
          image: "https://via.placeholder.com/300x200?text=Honda%20Civic",
          owner: "Sarah K.",
          location: "Astana, Downtown",
          available: true,
        },
        {
          id: "car3",
          ...
          year: 2023,
          price: 75,
          image: "https://via.placeholder.com/300x200?text=BMW%203Series",
          owner: "Alex M.",
        ...
        },
        {
          ...
          year: 2022,
          price: 80,
          image: "https://via.placeholder.com/300x200?text=Mercedes%20C-Class",
          owner: "Elena T.",
  ...
  // Обработка изменений (даты + платёж)
  const handleInputChange = (
    ...
    // Автоматический пересчёт дней и цены
  ...
  // Отправка (подтверждение бронирования)
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);

    try {
      // В реальном случае — отправка данных на сервер
      ...
      {availableCars.map((car) => (
        ...
        >
          ...
          <div className="mt-2 flex justify-between items-center">
            <span className="text-blue-400 font-bold">{car.price} ₸ / day</span>
            <span className="text-xs text-gray-400">Owner: {car.owner}</span>
          </div>
        </div>
      ))}
    </div>
  ...
  const renderDateSelection = () => (
    <div className="relative backdrop-blur-sm bg-black/40 border border-white/10 rounded-2xl p-8 shadow-2xl overflow-hidden">
      <h2 className="text-2xl font-bold text-white mb-6">
        Choose Dates for {selectedCar?.make} {selectedCar?.model}
      </h2>
  ...
  const renderConfirmation = () => (
    <div className="relative backdrop-blur-sm bg-black/40 border border-white/10 rounded-2xl p-8 shadow-2xl overflow-hidden text-center">
      ...
      <p className="text-gray-300 mb-6">
        Your booking for the <strong>{selectedCar?.make}</strong>{" "}
        <strong>{selectedCar?.model}</strong> has been successfully confirmed.
      </p>
      ...
      <div className="relative container mx-auto px-4 py-12">
        ...
        <div className="flex flex-col items-center mb-12">
          ...
          <p className="text-gray-400 text-lg max-w-2xl text-center">
            Choose your car, select dates, and confirm payment in just a few
            clicks
          </p>
        </div>
