...
@app.post("/bookings")
def create_booking(booking: Booking, user: dict = Depends(verify_token)):
    new_booking = {
        "id": len(bookings) + 1,
...
def list_bookings(user: dict = Depends(verify_token)):
    user_bookings = [b for b in bookings if b["renter"] == user["email"]]
    return {"bookings": user_bookings}

@app.get("/bookings/{booking_id}")
def get_booking(booking_id: int, user: dict = Depends(verify_token)):
    booking = next((b for b in bookings if b["id"] == booking_id and b["renter"] == user["email"]), None)
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    return {"booking": booking}

@app.put("/bookings/{booking_id}")
def update_booking(booking_id: int, booking: Booking, user: dict = Depends(verify_token)):
    existing_booking = next((b for b in bookings if b["id"] == booking_id and b["renter"] == user["email"]), None)
...
def delete_booking(booking_id: int, user: dict = Depends(verify_token)):
    global bookings
    if not any(b for b in bookings if b["id"] == booking_id and b["renter"] == user["email"]):
        raise HTTPException(status_code=404, detail="Booking not found")
    bookings = [b for b in bookings if not (b["id"] == booking_id and b["renter"] == user["email"])]
